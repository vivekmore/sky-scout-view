...
export const WindCompass = ({
  ...
  const compass = (
    <div
      className={cn(
        // Scale progressively to fit narrow screens. origin-top so vertical space is predictable.
        "relative w-80 h-80 max-w-full origin-top scale-[0.55] xs:scale-[0.6] sm:scale-[0.7] md:scale-[0.85] lg:scale-100 transition-transform",
        // Ensure it never overflows parent horizontally
        "[&>div]:max-w-full"
      )}
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none select-none font-extrabold tracking-wider text-foreground/90">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-[120%] text-2xl md:text-3xl drop-shadow-sm">
          N
        </div>
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-[120%] text-2xl md:text-3xl text-muted-foreground drop-shadow-sm">
          S
        </div>
        <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-[120%] text-2xl md:text-3xl text-muted-foreground drop-shadow-sm">
          W
        </div>
        <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-[120%] text-2xl md:text-3xl text-muted-foreground drop-shadow-sm">
          E
        </div>
      </div>
  ...
  const infoPanel = (
    <div className="text-center mt-6 md:mt-12" aria-live="polite">
      <div className="text-4xl sm:text-5xl md:text-7xl font-bold text-foreground">
      ...
      <div className="text-2xl sm:text-3xl md:text-3xl text-muted-foreground mt-1">
        {speed.toFixed(0)}
        <span className="text-lg sm:text-xl"> mph</span>
      </div>
    </div>
  );

  const content = (
    <div className="flex flex-col items-center gap-2 sm:gap-4 flex-1 justify-center w-full">
      {compass}
      {infoPanel}
    </div>
  );
  ...
  if (effectiveVariant === "unstyled") {
    return (
      <div className={cn("p-4 sm:p-6 h-full w-full flex justify-center", className)}>{content}</div>
    );
  }
  ...
  if (effectiveVariant === "panel") {
    return (
      <PanelContainer center noPadding className={className}>
        <div className="p-4 sm:p-6 h-full w-full flex justify-center">{content}</div>
      </PanelContainer>
    );
  }
  ...
  // default
  return (
    <Card className={cn("p-4 sm:p-6 h-full w-full flex justify-center", className)}>{content}</Card>
  );
};
